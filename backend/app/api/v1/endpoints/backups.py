...
Provides superuser-only endpoints for triggering, listing, and purging
database snapshots. Backups are recorded in the `backups` table for
history tracking in the Dashboard.
"""
import uuid
...
async def list_backups(
    ...
    """List all database snapshots recorded in the registry, newest first."""
...
async def trigger_backup(
    ...
    Executes pg_dump via BackupService then persists a Backup record
    so the Dashboard shows the new entry immediately.
    """
    try:
        ...
        # FIX: Backup model fields: filename, type (NOT NULL), size_bytes (NOT NULL),
...
async def delete_backup(
    ...
    try:
        ...
        # Allow filename deletion if id is not a valid UUID
